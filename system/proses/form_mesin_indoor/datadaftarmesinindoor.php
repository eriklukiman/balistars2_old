...
foreach($hasil as $row){
  ...
  $disabled='';
  $sqlCari=$db->prepare('SELECT idPerformaIndoor from balistars_performa_mesin_indoor where idPenjualanDetail=?');
  $sqlCari->execute([$row['idPenjualanDetail']]);
  $dataCari=$sqlCari->fetch();
  if($dataCari['idPerformaIndoor']>0){
    $disabled='disabled';
  }
  ?>
  <tr>
    ...
    <td>
      ...
      if($disabled==''){
       ...
       onclick="prosesMesinIndoor('<?=$row['idPenjualanDetail']?>')">
      <i class="fa fa-save"></i></button>
       ...
       onclick="editMesinIndoor('<?=$dataCari['idPerformaIndoor']?>')">
