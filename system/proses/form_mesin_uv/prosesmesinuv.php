...
if($flag == 'cancel'){
  $sql=$db->prepare('DELETE FROM balistars_performa_mesin_uv 
    where idPerformaUV=?');
  $hasil=$sql->execute([$idPerformaUV]);
}
else{
  ...
  if($flag == 'update'){
    $sql=$db->prepare('UPDATE balistars_performa_mesin_uv set 
      tanggalPerforma=?,
      namaBahan=?,
      luas=?,
      idUserEdit=?
    ...
    $sql=$db->prepare('INSERT INTO balistars_performa_mesin_uv set 
      idCabang=?,
      noNota=?,
      idPenjualanDetail=?,
      tanggalPerforma=?,
