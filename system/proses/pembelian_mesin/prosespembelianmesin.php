  ...
  if($flag=='update'){
    ...
    foreach($dataMesin as $data){
      ...
      for($i=1; $i<=$interval; $i++ ){
        ...
        $sql1=$db->prepare('INSERT INTO balistars_mesin_penyusutan set 
          idPembelianDetail=?,
          noNota=?,
          kodeAkunting=?,
          tanggalPenyusutan=?,
          nilaiPenyusutan=?,
          idUser=?');
        $hasil1=$sql1->execute([$idPembelianDetail,$noNota,$kodeAkunting,$tanggal,$nilaiPenyusutan,$idUserAsli]);
        $tanggalPembelian=bulanBesok($tanggal);
      }
    }
  }
  else{
    $sql=$db->prepare('INSERT INTO balistars_pembelian_mesin set  
      noNota=?,
      tipePembelian=?,
      noNotaVendor=?,
      lamaPenyusutan=?,
    ...
    $hasil2='';
    $sqlMesin=$db->prepare('SELECT * FROM 
    balistars_pembelian_mesin_detail 
