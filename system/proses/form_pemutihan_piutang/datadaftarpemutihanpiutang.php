...
foreach($hasil as $row){
  ...
  <tr>
    ...
    <td>
      <button type               = "button" 
              title              = "Edit"
              class              = "btn btn-warning tombolEditPemutihan" 
              style              = "color: white;"
              onclick = "editPemutihanPiutang('<?=$row['idPemutihan']?>')" <?=$disabled1?> >
        <i class="fa fa-edit"></i>
      </button>
      <button type    = "button"
              title   = "Hapus" 
              class   = "btn btn-danger" 
              onclick = "cancelPemutihanPiutang('<?=$row['idPemutihan']?>')" <?=$disabled2?> 
              >
        <i class="fa fa-trash"></i>
      </button>
    </td>
    <td ><?=wordwrap(ubahTanggalIndo($row['tanggalPemutihan']),50,'<br>')?></td>
    ...
    <td>Rp <?=wordwrap(ubahToRp($row['sisaPiutang']),50,'<br>')?></td>
  </tr>
  <?php
  $n++;
}
