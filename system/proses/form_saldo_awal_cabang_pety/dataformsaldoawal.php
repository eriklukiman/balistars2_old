...
$idCabangCashKecil = '';
$readonly      = '';
$tanggalCabangCashKecil = date('Y-m-d');
extract($_REQUEST);
...
$sqlUpdate  = $db->prepare('SELECT * from balistars_cabang_cash_kecil
  where idCabangCashKecil = ?');
$sqlUpdate->execute([$idCabangCashKecil]);
$dataUpdate = $sqlUpdate->fetch();

if($dataUpdate){
  $tanggalCabangCashKecil = $dataUpdate['tanggalCabangCashKecil']??'';
...
<form id="formSaldoAwal">
  ...
  <input type="hidden" name="idCabangCashKecil"  value="<?=$idCabangCashKecil?>">
  ...
  <div class="form-group row">
    <div class="col-sm-6">
      ...
      <div class="input-group date"  data-date-autoclose="true" data-provide="datepicker"  data-date-format="yyyy-mm-dd">
        <input type="tanggal" class="form-control" name="tanggalCabangCashKecil" id="tanggalCabangCashKecil" value="<?=$tanggalCabangCashKecil?>"  autocomplete="off">
