...
$namaPegawai = explode(' ', $dataLogin['namaPegawai']);
  ...
  $dataPenjualan=executeQueryUpdateForm('SELECT * FROM balistars_penjualan where noNota=?',$db,$noNota);
  $dataPiutang=executeQueryUpdateForm('SELECT * FROM balistars_piutang where noNota=? order by idPiutang',$db,$noNota);
  $dataCabang=executeQueryUpdateForm('SELECT * FROM balistars_cabang where idCabang=?',$db,$dataPenjualan['idCabang']);
  ...
  $dataUser=executeQueryUpdateForm('SELECT * FROM balistars_user where idUser=?',$db,$dataPenjualan['idUser']);
?>
<!DOCTYPE html>
<html lang="en">
    <head>
