...
if (!$dataCekUser || !$dataCekMenu) {
    ...
    $dataTahapanSebelumReject = selectStatement(
        ...
        [$idPettyCash, $tahapanSebelumnya, 'Petty Cash', 'Aktif'],
        'fetch'
    );
    ...
    if ($status == true) {
        $statusTahapan = updateStatement(
            ...
            WHERE
                idPettyCash = ?',
            [$tahapanBaru, $idPettyCash]
        );
    }

    if ($hasil === 'Reject') {
        if ($status) {
            $pesan = 'Proses Reject Petty Cash Berhasil';
        } else {
            $pesan = 'Proses Reject Petty Cash Gagal';
        }
    } else if ($hasil === 'Disetujui') {
        if ($status) {
            $pesan = 'Proses Penyetujuan Petty Cash Berhasil';
        } else {
            $pesan = 'Proses Penyetujuan Petty Cash Gagal';
        }
    }
