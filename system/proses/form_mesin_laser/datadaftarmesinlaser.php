...
$sqlPerforma=$db->prepare('
  ...
  and idCabang=? 
  order by tanggalPerforma DESC, idPerformaLaser DESC');
$sqlPerforma->execute([
  $tanggalAwal,
  $tanggalAkhir,
  $dataLogin['idCabang']]);
  ...
  <tr>
    ...
    <td>
     <!--  <button type               = "button" 
              ...
              style              = "color: white;"
              onclick = "editMesinLaser('<?=$idPerformaLaser?>')" <?=$disabled1?>>
      ...
      <button type    = "button"
              ...
              onclick = "cancelMesinLaser('<?=$idPerformaLaser?>')" <?=$disabled2?>>
    ...
    <td><?=ubahToRp($jumlahKlik,50,'<br>')?></td>
  </tr>
  <?php
  $totalKlik+=($row['jumlahKlik']);
  }
  $tanggalPerforma=$row['tanggalPerforma'];
  $idPerformaLaser=$row['idPerformaLaser'];
