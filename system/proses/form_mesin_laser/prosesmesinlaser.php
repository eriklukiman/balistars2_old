...
if($flag == 'cancel'){
  $sql=$db->prepare('DELETE FROM balistars_performa_mesin_laser 
    where idPerformaLaser=?');
  $hasil=$sql->execute([$idPerformaLaser]);
}
else{
  ...
  if($flag == 'stop'){
    $sql=$db->prepare('INSERT INTO balistars_performa_mesin_laser set 
      idCabang=?,
      tanggalPerforma=?,
      klikBefore=?,
      jumlahKlik=?,
