...
foreach($hasil as $row){
  ...
  $disabled='';
  $sqlCari=$db->prepare('SELECT idPerformaOutdoor from balistars_performa_mesin_outdoor where idPenjualanDetail=?');
  $sqlCari->execute([$row['idPenjualanDetail']]);
  $dataCari=$sqlCari->fetch();
  if($dataCari['idPerformaOutdoor']>0){
    $disabled='disabled';
  }
  ?>
  <tr>
    ...
    <td>
      ...
      if($disabled==''){
       ...
       onclick="prosesMesinOutdoor('<?=$row['idPenjualanDetail']?>')">
      <i class="fa fa-save"></i></button>
       ...
       onclick="editMesinOutdoor('<?=$dataCari['idPerformaOutdoor']?>')">
