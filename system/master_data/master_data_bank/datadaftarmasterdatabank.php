  ...
  <tr>
    ...
    <td>
      <button type               = "button" 
              ...
              style              = "color: white;"
              onclick = "editBank('<?=$row['idBank']?>')" <?=$disabled1?> >
        <i class="fa fa-edit"></i>
      </button>
      <button type    = "button"
              title   = "Hapus" 
              class   = "btn btn-danger" 
              onclick = "cancelBank('<?=$row['idBank']?>')" <?=$disabled2?> 
              >
        <i class="fa fa-trash"></i>
      </button>
    </td>
    <td><?=wordwrap($row['noRekening'],50,'<br>')?></td>
    <td><?=wordwrap($row['namaBank'],50,'<br>')?></td>
    <td><?=wordwrap($row['atasNama'],50,'<br>')?></td>
    <td><?=wordwrap($row['tipe'],50,'<br>')?></td>
  </tr>
  <?php
  $n++;
}
