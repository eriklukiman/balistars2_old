...
$sql->execute([$idUserAsli,'Master Jabatan']);
$data=$sql->fetch();

$sqlJabatan  = $db->prepare('SELECT * from balistars_jabatan
 where statusJabatan = ? 
...
foreach($dataJabatan as $row){
  ...
  <tr>
    ...
    <td>
      <button type               = "button" 
              title              = "Edit"
              class              = "btn btn-warning tombolEditJabatan" 
              style              = "color: white;"
              onclick = "editJabatan('<?=$row['idJabatan']?>')" <?=$disabled1?> >
        <i class="fa fa-edit"></i>
      </button>
      <button type    = "button"
              title   = "Hapus" 
              class   = "btn btn-danger" 
              onclick = "cancelJabatan('<?=$row['idJabatan']?>')" <?=$disabled2?>>
