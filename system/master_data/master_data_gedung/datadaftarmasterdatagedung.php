...
$sql->execute([$idUserAsli,'Master Gedung']);
$data=$sql->fetch();

$sqlGedung  = $db->prepare('SELECT * from balistars_gedung inner join balistars_cabang on balistars_gedung.idCabang=balistars_cabang.idCabang
 where statusGedung = ? 
  order by namaGedung');
...
foreach($dataGedung as $row){
  ...
  <tr>
    ...
    <td>
      <button type               = "button" 
              title              = "Edit"
              class              = "btn btn-warning tombolEditGedung" 
              style              = "color: white;"
              onclick = "editGedung('<?=$row['idGedung']?>')" <?=$disabled1?>>
        <i class="fa fa-edit"></i>
      </button>
      <button type    = "button"
              title   = "Hapus" 
              class   = "btn btn-danger" 
              onclick = "cancelGedung('<?=$row['idGedung']?>')" <?=$disabled2?>>
