...
if($flag == 'cancel'){
  $sql = $db->prepare('UPDATE balistars_cabang set statusCabang = ?, idUserEdit=? where idCabang = ?');
  $hasil = $sql->execute(['Non Aktif',$idUserAsli, $idCabang]);
}
else if($flag == 'update'){
  $sql = $db->prepare('UPDATE balistars_cabang set 
    namaCabang    = ?,
    alamatCabang    = ?,
    kota  = ?,
    area = ?,
  ...
  $sql = $db->prepare('INSERT INTO balistars_cabang set 
    namaCabang    = ?,
    alamatCabang    = ?,
    kota  = ?,
    area = ?,
