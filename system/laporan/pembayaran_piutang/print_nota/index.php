...
<html lang="en">
  ...
  //Akses Piutang
  $sqlPiutang=$db->prepare('SELECT * FROM balistars_piutang where noNota=? order by idPiutang ASC');
  $sqlPiutang->execute([$noNota]);
  $hasilPiutang=$sqlPiutang->fetchAll();
        ...
        <table style="position: relative; <?=$stylePage?>">
          ...
          <tbody>
            ...
            ?>
              ...
              <?php 
              $n=1;
              foreach($hasilPiutang as $data){
                ?>
                <tr>
                  <td style="vertical-align:bottom; text-align: right;">DP<?=$n?> Tanggal <?=$data['tanggalPembayaran']?> :</td>
                  <td style="vertical-align:bottom; text-align: right;"><?=ubahToRp($data['jumlahPembayaran'])?></td>
                </tr>
                <?php  
                if($n==(count($hasilPiutang))){ 
                  ?>
                  <tr>
                    <td style="vertical-align:bottom; text-align: right;">Sisa Pembayaran :</td>
                    <td style="vertical-align:bottom; text-align: right;"><?=ubahToRp($data['sisaPiutang'])?></td>
                  </tr>
                  <?php
                }
                ?>
