...
<div style="padding-bottom: 20px">
  <b>Target Achievement Area : <?=ubahToRp($dataAchievement['jumlahAchievement'])?></b>
</div>
<table class="table table-bordered table-custom">
  <thead class="bg-info text-white">
    ...
    <th>
      <button class="btn btn-sm btn-info" onclick="dataDaftarAchievementAreaAkm()">
        Tanggal
      </button>
    </th>
    ...
    <?php 
      ...
      for ($i=1; $i <= $tanggalPecah[2] ; $i++) { 
        ...
        $sqlPenjualan->execute([
          $area,
          $tanggalAwal,$tanggalAwalJalan,
          "Aktif"]);
        $dataPenjualan=$sqlPenjualan->fetch();

        $acv=$dataPenjualan['achievement']
            +fungsiPenyesuaian($db,$tanggalAwal,$tanggalAwalJalan,$area,'Penjualan');
      ?>
        <tr>
          <td><?=$n?></td>
          ...
          <td>
            <?php
            if($dataAchievement){
              $ratio=round($acv/$dataAchievement['jumlahAchievement']*100);
            }
            else{
              $ratio='NAN';
            }
