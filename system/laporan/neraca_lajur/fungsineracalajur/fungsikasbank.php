...
function kasBank($total,$db,$tanggalAwal,$tanggalAkhir,$tipe)
{
	...
	$execute = array(
		...
		$tanggalAwalSaldo,$tanggalAkhirSaldo,'final',
		// $tanggalAwalSaldo,$tanggalAkhirSaldo,'Disetujui',
		$tanggalAwalSaldo,$tanggalAkhirSaldo,'Giro','Cair', 
		...
		$tanggalAwal,$tanggalAkhir,'final',30,22,23,24,25,33,37,
		$tanggalAwal,$tanggalAkhir,'approved',0,
	...
	$sql 	 = $db->prepare('
        ...
        (
          ...
          AND statusTransfer=? 
          AND idBankTujuan not in (?,?,?,?,?,?,?)
          GROUP BY idBankTujuan
        )
        UNION ALL
        (
          SELECT (SUM(jumlahPembayaran)-SUM(COALESCE(biayaAdmin,0))-SUM(COALESCE(PPH,0))) AS debet, 0 AS kredit, balistars_piutang.bankTujuanTransfer AS idBank 
        ...
        UNION ALL
        -- (
        --   SELECT 0 AS debet, SUM(nilaiDisetujui) AS kredit, idBankTransfer AS idBank 
        --   FROM balistars_advertising_rab
        --     WHERE (tanggalPengajuan BETWEEN ? AND ?) AND statusRAB=?
        --     GROUP BY idBankTransfer
        -- )
        -- UNION ALL
        (
          SELECT 0 AS debet, SUM(jumlahPembayaran) AS kredit, bankAsalTransfer AS idBank 
		...
		(
			...
			(
				...
				UNION ALL
				(
				SELECT 0 AS debet, SUM(nilaiTransfer) AS kredit, idBankAsal AS idBank FROM balistars_bank_transfer
			    WHERE (tanggalTransfer BETWEEN ? AND ?) 
			    AND statusTransfer=? 
			    AND idBankAsal not in (?,?,?,?,?,?,?)
			    GROUP BY idBankAsal
				)
				UNION ALL
				(
