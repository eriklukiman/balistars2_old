...
function kasHutangA2($total,$db,$tanggalAwal,$tanggalAkhir,$tipe)
{
	...
	$execute = array(
		...
		$tanggalAwalSaldo,$tanggalAkhirSaldo,
        $tanggalAwalSaldo,$tanggalAkhirSaldo, 'A2',
        $tanggalAwalSaldo,$tanggalAkhirSaldo, 'A2', 'DP', 'Aktif', 
        $tanggalAwalSaldo,$tanggalAkhirSaldo, 'A2', 'DP', 'Aktif', 
		...
		$tanggalAwal,$tanggalAkhir,
        $tanggalAwal,$tanggalAkhir, 'A2',
        $tanggalAwal,$tanggalAkhir, 'A2', 'DP', 'Aktif', 
        $tanggalAwal,$tanggalAkhir, 'A2', 'DP', 'Aktif', 
	...
	$sql 	 = $db->prepare('
        ...
        (
        	...
        	where (balistars_hutang_mesin.tanggalPembayaran BETWEEN ? and ?) 
            and statusCair="Cair"  
        	and tipePembelian = ? 
        	and statusPembelianMesin="Aktif"
        )
        ...
        UNION ALL
        (
                SELECT CASE WHEN jenisPPN = "Include" 
                THEN (hargaSatuan*100/110)*qty 
                ELSE hargaSatuan*qty END AS kredit, 0 as debet, tipePembelian as tipe 
            FROM balistars_pembelian_mesin_detail
                inner join balistars_pembelian_mesin
                on balistars_pembelian_mesin_detail.noNota=balistars_pembelian_mesin.noNota
                where (tanggalPembelian BETWEEN ? and ?) 
                and tipePembelian =? 
            and kodeAkunting in ("1313","1314","1316")
                and statusPembelianMesin="Aktif"
        )
        UNION all
        (
